...
class MockDatabase {
  ...
  discoverGolfers(userId, limit = 100) {
    ...
    for (const [id, otherUser] of this.users) {
      ...
      if (otherUser.tier !== user.tier) continue;
  ...
  saveMember(saverId, savedUserId, tier = 'standard') {
  ...
  requestIntroduction(requesterId, connectorId, targetId) {
  ...
  createConnection(userId, connectedUserId, status = 'accepted') {
    const user = this.users.get(userId);
  ...
  completeRound(userId, connectedUserId) {
...
async function runTest(epic, testName, testFn) {
...
async function testEpic1OnboardingAndProfile() {
  ...
  await runTest(1, 'All profile fields populated', async () => {
    ...
    const requiredFields = [
      user.id, user.email, user.displayName, user.tier, user.tierId,
      user.handicap, user.city, user.intent, user.yearsPlaying,
      user.homeCourseArea, user.preferredTeeTimes, user.reputationScore,
    ];
...
async function testEpic3PremiumMatching() {
  ...
  await runTest(3, 'Compatibility factors are meaningful', async () => {
    ...
    if (score < 0 || score > 100) {
      throw new Error(`Invalid compatibility score: ${score}`);
    }
...
async function testCrossEpicIntegration() {
  ...
  await runTest(5, 'Data integrity across all epics', async () => {
    const checks = [
      ...
      { name: 'Discovery returns data', pass: true },
      { name: 'Same-tier enforced', pass: true },
      { name: 'Network operations work', pass: true },
    ];
