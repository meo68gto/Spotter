...
import { OperatorErrorBoundary } from "@/components/(operator)/ErrorBoundary";
...
export default function OperatorLayout({ children }: OperatorLayoutProps) {
  ...
  const isActive = (path: string) => {
    if (path === "/operator-dashboard") {
      return pathname === "/operator-dashboard";
    }
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
        ...
        <nav className="flex-1 p-4">
          <ul className="space-y-2">
            <li>
              <Link href="/operator-dashboard" className={navLinkClass("/operator-dashboard")}>
        ...
        <div className="p-4 border-t border-gray-200">
          <div className="flex items-center">
            <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center">
              <span className="text-sm font-medium text-indigo-700">OP</span>
            </div>
            <div className="ml-3">
