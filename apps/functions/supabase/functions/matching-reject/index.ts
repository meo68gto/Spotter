...
Deno.serve(async (req) => {
  ...
  if (match.status !== 'pending') {
    return badRequest('Only pending matches can be rejected', 'invalid_match_state');
  }
  if (match.candidate_user_id !== user.id && match.requester_user_id !== user.id) {
    return badRequest('Not a match participant', 'participant_mismatch');
  }
  ...
  if (error) return json(500, { error: error.message, code: 'match_reject_failed' });
